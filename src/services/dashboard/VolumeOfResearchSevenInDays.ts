...
class VolumeOfResearchSevenInDays
{
	public async execute({ company, store }: CompanyType)
	{
		if(typeof store === 'undefined') {
			...
			and question.company_id = ? order by answer.created_at desc;`, [company]);
			...
			and store.company_id = ? and store.store_number = ? order by answer.created_at desc;`, [company, store]);
