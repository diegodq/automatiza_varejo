...
export class default1681433080605 implements MigrationInterface {
    name = 'default1681433080605'
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE \`customer\` (\`id\` int NOT NULL AUTO_INCREMENT, \`first_name\` varchar(200) NOT NULL, \`avatar\` varchar(255) NULL, \`surname\` varchar(255) NOT NULL, \`position\` varchar(255) NOT NULL, \`phone\` varchar(17) NOT NULL, \`email\` varchar(255) NULL, \`activated\` tinyint NULL DEFAULT '0', \`activated_on\` datetime NULL, \`accept_newsletter\` tinyint NULL DEFAULT '0', \`info_payment\` tinyint NULL DEFAULT '0', \`accept_terms\` char NOT NULL, \`old_password\` varchar(255) NULL, \`password\` varchar(255) NOT NULL, \`system_user\` varchar(40) NULL, \`agent_user\` varchar(40) NULL, \`pass_change_on\` datetime NULL, \`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), UNIQUE INDEX \`IDX_fdb2f3ad8115da4c7718109a6e\` (\`email\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
