...
export class default1684346406438 implements MigrationInterface {
  name = 'default1684346406438'
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(`CREATE TABLE \`customer\` (\`id\` int NOT NULL AUTO_INCREMENT, \`first_name\` varchar(200) NOT NULL, \`avatar\` varchar(255) NULL, \`surname\` varchar(255) NOT NULL, \`position\` varchar(255) NOT NULL, \`phone\` varchar(17) NOT NULL, \`email\` varchar(255) NULL, \`temp_email\` varchar(255) NULL, \`email_change_on\` datetime NULL, \`activated\` tinyint NULL DEFAULT '0', \`activated_on\` datetime NULL, \`accept_newsletter\` tinyint NULL DEFAULT '0', \`info_payment\` tinyint NULL DEFAULT '0', \`accept_terms\` char NOT NULL, \`accept_terms_on\` datetime NULL, \`old_password\` varchar(255) NULL, \`password\` varchar(255) NOT NULL, \`system_user\` varchar(40) NULL, \`agent_user\` varchar(40) NULL, \`pass_change_on\` datetime NULL, \`city\` varchar(90) NULL, \`country_name\` varchar(90) NULL, \`country_capital\` varchar(90) NULL, \`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), UNIQUE INDEX \`IDX_fdb2f3ad8115da4c7718109a6e\` (\`email\`), UNIQUE INDEX \`IDX_7f0a0b82d3e4b38ace0200652c\` (\`temp_email\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
    ...
    await queryRunner.query(`CREATE TABLE \`question\` (\`id\` int NOT NULL AUTO_INCREMENT, \`title_question\` varchar(100) NULL, \`question_description\` varchar(100) NULL, \`status\` tinyint NULL, \`tree_question\` varchar(10) NOT NULL, \`type_question\` varchar(20) NOT NULL, \`option_one\` varchar(10) NULL, \`option_two\` varchar(10) NULL, \`import_type\` varchar(15) NULL, \`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`department_id\` int NULL, \`company_id\` int NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
    await queryRunner.query(`CREATE TABLE \`topic\` (\`id\` int NOT NULL AUTO_INCREMENT, \`name\` varchar(50) NOT NULL, \`status\` tinyint NOT NULL DEFAULT '1', \`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`companyId\` int NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
    ...
    await queryRunner.query(`ALTER TABLE \`question\` ADD CONSTRAINT \`FK_6327ca664ca925b73b5e12317d9\` FOREIGN KEY (\`department_id\`) REFERENCES \`department\`(\`id\`) ON DELETE CASCADE ON UPDATE CASCADE`);
    ...
    await queryRunner.query(`ALTER TABLE \`topic\` ADD CONSTRAINT \`FK_504a32688d181c7a5b1d815942f\` FOREIGN KEY (\`companyId\`) REFERENCES \`company\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`);
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(`ALTER TABLE \`topic\` DROP FOREIGN KEY \`FK_504a32688d181c7a5b1d815942f\``);
    await queryRunner.query(`ALTER TABLE \`question\` DROP FOREIGN KEY \`FK_150758ea0fe6f96dd2cd53bff21\``);
    await queryRunner.query(`ALTER TABLE \`question\` DROP FOREIGN KEY \`FK_6327ca664ca925b73b5e12317d9\``);
